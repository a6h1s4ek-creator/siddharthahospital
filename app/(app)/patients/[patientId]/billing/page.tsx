...
import { addBillingItem, updateBillingStatus, deleteBillingItem, type Patient, type BillingItem } from '@/lib/firebase-services';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function PatientBillingPage() {
  ...
  const { data: patient, loading: patientLoading, error: patientError } = useDoc<Patient>('patients', patientId);
  ...
  const handleAddBillingItem = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const newBillingItem: Omit<BillingItem, 'id'> = {
    ...
    try {
        await addBillingItem(patient.id, newBillingItem, patient.billing || []);
        toast({
  ...
  const handleStatusChange = async (itemId: string) => {
    if (!patient || !patient.billing) return;

    const currentItem = patient.billing.find(item => item.id === itemId);
    if (!currentItem) return;

    const newStatus = currentItem.status === 'Paid' ? 'Unpaid' : 'Paid';
    
    try {
        await updateBillingStatus(patient.id, itemId, newStatus, patient.billing);
        toast({
  ...
  const handleDeleteItem = async (itemId: string, itemService: string) => {
    if (!patient || !patient.billing) return;

    try {
        await deleteBillingItem(patient.id, itemId, patient.billing);
        toast({
            title: 'Billing Item Removed',
            description: `${itemService} has been removed from the bill.`
        });
    } catch (error) {
        ...
        toast({
            ...
            description: 'Could not remove billing item.',
        });
    }
  };
      ...
      <div className="flex min-h-screen w-full flex-col">
        ...
        <main className="flex flex-1 items-center justify-center p-4 md:p-8">
          <Card>
            ...
            <CardContent>
              <p>{patientError ? patientError.message : 'The requested patient could not be found.'}</p>
            </CardContent>
          </Card>
        </main>
      </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center gap-4">
             <Avatar className="h-16 w-16">
                <AvatarImage src={patient.avatar.imageUrl} alt={patient.name} data-ai-hint={patient.avatar.imageHint} />
                <AvatarFallback>{patient.name.charAt(0)}</AvatarFallback>
            </Avatar>
            <div>
                <CardTitle className="text-3xl">{patient.name}</CardTitle>
                <CardDescription>
                    Patient ID: {patient.patientId} &bull; Age: {patient.age} &bull; Gender: {patient.gender}
                </CardDescription>
            </div>
                                       ...
                                       <AlertDialog>
                                          ...
                                          <AlertDialogContent>
                                            <AlertDialogHeader>
                                              ...
                                              <AlertDialogDescription>
                                                This action cannot be undone. This will permanently remove the "{item.service}" item from the patient's bill.
                                              </AlertDialogDescription>
                                            </AlertDialogHeader>
                                            <AlertDialogFooter>
                            ...
                            {labTestEntries.map((entry, index) => (
                               <div key={entry.id} className="flex items-start gap-2">
                                <Popover open={entry.popoverOpen} onOpenChange={(isOpen) => updateLabTestEntry(entry.id, { popoverOpen: isOpen })}>
                                    ...
                                    <PopoverContent className="w-56 md:w-[250px] p-0">
                                    <Command>
                                        <CommandInput
